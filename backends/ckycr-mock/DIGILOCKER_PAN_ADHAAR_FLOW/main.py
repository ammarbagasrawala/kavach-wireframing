...
async def start_digilocker(request_data: Optional[StartDigilockerRequest] = None):
    ...
    # If using IDfy's online webhook tool (webhooks.idfy.tech), use that exact URL
    callback_url = os.getenv("IDFY_WEBHOOK_URL")
    if not callback_url:
        # Otherwise, default to the local or ngrok base URL + our endpoint
    ...
    payload = {
        ...
        "data": {
            ...
            # IDfy forcefully strips ports (like :8000) from redirect URLs for security, 
            # so we MUST use the ngrok URL to safely return the user to the app!
            "redirect_url": f"{webhook_base.rstrip('/')}/?task_id={task_id}",
